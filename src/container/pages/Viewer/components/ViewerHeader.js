...
const ViewerHeader = () => {
  ...
  const { addCsTool, addTextTool, reset, openFullScreen, scheduleMeeting, showContactList, onSelectPreset } =
    useViewer();
  return (
    <div className="viewer-panel flex flex-row overflow-x-auto items-center gap-4 w-full px-4 h-16">
      {tools(addCsTool, addTextTool, reset, openFullScreen, scheduleMeeting, showContactList, onSelectPreset).map(
        (tool, i) =>
